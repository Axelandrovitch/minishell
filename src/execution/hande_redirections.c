...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   hande_redirections.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ahetru <marvin@42.fr>                      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/14 12:57:42 by ahetru            #+#    #+#             */
/*   Updated: 2025/05/14 12:57:43 by ahetru           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_fd(enum e_token_type type, char *filename)
{
	int	fd;
...
int	handle_redirections(t_command_block *command_block)
{
	...
	while (token)
	{
		if (is_redirection(token))
		{
			...
			{
				perror("open");
				printf("DEBUG PRINT: Remember to clean befor exit\n");
				exit(1);
			}
