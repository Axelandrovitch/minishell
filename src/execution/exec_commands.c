...
// 		close(fd[1]);
// 	}
// 	apply_redirections(block->redirs, shell);
...
// }
//
// bool	has_heredoc(t_redir *redirs)
// {
// 	while (redirs)
// 	{
...
void	update_parent_state(pid_t pid, int *fd, int *prev_fd, bool has_next)
{
	if (*prev_fd != -1)
		close(*prev_fd);
	if (has_next)
...
void	update_parent_wrapper(pid_t pid, int *fd, int *prev_fd, t_command_block *next)
{
	if (next != NULL)
		update_parent_state(pid, fd, prev_fd, 1);
	else
		update_parent_state(pid, NULL, prev_fd, 0);
}
...
void	exec_commands(t_command_block *cmd, int prev_fd, t_shell *shell)
{
	...
	while (cmd)
	{
		...
		if (pid == 0)
			...
			update_parent_wrapper(pid, fd, &prev_fd, cmd->next);
		cmd = cmd->next;
	}
}
